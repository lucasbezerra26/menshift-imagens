...
def remove_leishmania_recorte(start_h, end_h, start_w, end_w):
    ...
    if (qnt_labels_last_crop > 0):
        ...
        if (temp_mask[start_h:end_h, start_w:end_w].max() == True):
            return np.expand_dims(temp_mask[start_h:end_h, start_w:end_w].astype(float),
                                  axis=-1), 1  # converte para float e expande a dimensão
        else:
...
def recorta_imagens(img, mask_geral, img_cont):
    ...
    while ((start_h + IMG_ALTURA) <= img.shape[0]):
        ...
        while ((start_w + IMG_LARGURA) <= img.shape[1]):
            ...
            # dataset = dataset.concat({'img':img_recorte, 'mask': mask_recorte, 'img_id': img_cont, 'label': label}, ignore_index=True)
            dataset = pd.concat([dataset, pd.DataFrame(
                {'img': [img_recorte], 'mask': [mask_recorte], 'img_id': [img_cont], 'label': [label]})],
                                ignore_index=True)
            ...
            if ((start_w + IMG_LARGURA) > img.shape[1]):
                ...
                if (qnt_labels_last_crop) > 0:
                    ...
                    mask_recorte, label = remove_leishmania_recorte(start_h, end_h, img.shape[1] - IMG_LARGURA,
                                                                    img.shape[1])
                    # dataset = dataset.concat({'img':img_recorte, 'mask': mask_recorte, 'img_id': img_cont, 'label': label}, ignore_index=True)
                    dataset = pd.concat([dataset, pd.DataFrame(
                        {'img': [img_recorte], 'mask': [mask_recorte], 'img_id': [img_cont], 'label': [label]})],
        ...
        if ((start_h + IMG_ALTURA) > img.shape[0]):
            ...
            if (qnt_labels_last_line) > 0:
                ...
                while ((start_w + IMG_LARGURA) <= img.shape[1]):
                    ...
                    mask_recorte, label = remove_leishmania_recorte(img.shape[0] - IMG_ALTURA, img.shape[0], start_w,
                                                                    end_w)
                    # dataset = dataset.concat({'img':img_recorte, 'mask': mask_recorte, 'img_id': img_cont, 'label': label}, ignore_index=True)
                    dataset = pd.concat([dataset, pd.DataFrame(
                        {'img': [img_recorte], 'mask': [mask_recorte], 'img_id': [img_cont], 'label': [label]})],
                    ...
                    if ((start_w + IMG_LARGURA) > img.shape[1]):
                        ...
                        if (qnt_labels_last_crop) > 0:
                            ...
                            mask_recorte, label = remove_leishmania_recorte(img.shape[0] - IMG_ALTURA, img.shape[0],
                                                                            img.shape[1] - IMG_LARGURA, img.shape[1])
                            # dataset = dataset.concat({'img':img_recorte, 'mask': mask_recorte, 'img_id': img_cont, 'label': label}, ignore_index=True)
                            dataset = pd.concat([dataset, pd.DataFrame(
                                {'img': [img_recorte], 'mask': [mask_recorte], 'img_id': [img_cont],
...
def load_crop_images(PATH_IMAGENS_,
    ...
    IMG_PASSO_DINAMICO_ = IMG_LARGURA_ // 16
